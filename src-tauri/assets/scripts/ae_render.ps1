...
$aerender = $null
$aeFolder = Get-ChildItem "C:\Program Files\Adobe" -Directory -Filter "Adobe After Effects*" -ErrorAction SilentlyContinue |
    Sort-Object Name -Descending | Select-Object -First 1
if ($aeFolder) {
    $candidate = Join-Path $aeFolder.FullName "Support Files\aerender.exe"
    if (Test-Path $candidate) { $aerender = $candidate }
}

if ($aerender) {
    Start-Process -FilePath $aerender -ArgumentList "-project `"$Path`" -continueOnMissingFootage"
    exit
}

# aerender not found — show error dialog
...
"@
[DwmHelper]::SetCurrentProcessExplicitAppUserModelID("UFB.AERender") | Out-Null
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <Grid Margin="20">
        ...
        <TextBlock Grid.Row="0" Text="aerender.exe not found. Install After Effects to use this feature." Foreground="$fg"
