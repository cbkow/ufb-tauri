param(
    [string]$Title = "New Project Folder",
    [string]$TargetDir
)
...
"@
[DwmHelper]::SetCurrentProcessExplicitAppUserModelID("UFB.UnionFolders") | Out-Null
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <Grid Margin="20">
        ...
        <TextBlock Grid.Row="0" Text="Enter folder name:" Foreground="$fg"
                   FontFamily="Segoe UI" FontSize="14" Margin="0,0,0,10"/>
        <TextBox Grid.Row="1" x:Name="FolderName"
...
$nameBox = $window.FindName("FolderName")
...
if ($window.ShowDialog() -and $nameBox.Text.Trim()) {
    ...
    # Find next available version letter (a-z) based on prefix only
    $versionLetter = $null
    foreach ($c in 97..122) {  # ASCII a-z
        $prefix = "${dateStamp}$([char]$c)_"
        $existing = Get-ChildItem -Path $TargetDir -Directory -Filter "${prefix}*" -ErrorAction SilentlyContinue
        if (!$existing) {
            $versionLetter = [char]$c
            break
        }
    }

    if ($versionLetter) {
        $finalName = "${dateStamp}${versionLetter}_${folderName}"
        New-Item -ItemType Directory -Path (Join-Path $TargetDir $finalName) -Force | Out-Null
    }
}
